...
if [ "$(uname)" == "Darwin" ]; then
    ...
    if [ "$(which apt)" != "" ]; then
        ...
        echo "Updating apt..."
        sudo $INSTALL_PKG update && $INSTALL_PKG upgrade || quit_if_failed "apt update && apt upgrade"
    elif [ "$(which apt-get)" != "" ]; then
        echo "Using apt-get as package manager..."
        PKG_MGR="apt-get"
        INSTALL_PKG="sudo $PKG_MGR install"
        echo "Updating apt-get..."
        sudo $INSTALL_PKG update && $INSTALL_PKG upgrade || quit_if_failed "apt-get update && apt-get upgrade"
    elif [ "$(which yum)" != "" ]; then
        echo "Using yum as package manager..."
        PKG_MGR="yum"
        INSTALL_PKG="sudo $PKG_MGR install"
        echo "Updating yum..."
        sudo $INSTALL_PKG update && $INSTALL_PKG upgrade || quit_if_failed "yum update && yum upgrade"
    elif [ "$(which dnf)" != "" ]; then
        echo "Using dnf as package manager..."
        PKG_MGR="dnf"
        INSTALL_PKG="sudo $PKG_MGR install"
        echo "Updating dnf..."
        sudo $INSTALL_PKG update && $INSTALL_PKG upgrade || quit_if_failed "dnf update && dnf upgrade"
    elif [ "$(which pacman)" != "" ]; then
        echo "Using pacman as package manager..."
        PKG_MGR="pacman"
        INSTALL_PKG="sudo $PKG_MGR -S"
        echo "Updating pacman..."
        sudo $INSTALL_PKG -Syu
...
cp .cache .cargo .config .p10k.zsh .zshrc  ~/
    ...
    1|*)
        if [ "$(which starship)" == "" ]; then
            ...
            if [ "$(which pacman)" != "" ] || [ "$(which paru)" != "" ]; then
                ...
                echo "Installing starship prompt using curl..."
